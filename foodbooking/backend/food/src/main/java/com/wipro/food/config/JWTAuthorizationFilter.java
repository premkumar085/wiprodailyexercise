...
public class JWTAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            System.out.println("in the filter internal: " + path);

            // Skip JWT check for login and food endpoints
            if (path.startsWith("/user/login") || path.startsWith("/food")) {
                filterChain.doFilter(request, response);
                return;
            }

            // Only run JWT validation for secured endpoints
